	...
	public LoginPage() {
		...
		button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {
				System.out.println("Login button works"); // test if button works
				// if passcode is correct, login to server
				User users = new User();
				try {
					String passStr = new String(passcode.getPassword());
					access = users.parseFile(username.getText(), passStr);
					// if true
					if (access == true) {
						panel1.remove(username);
						panel1.remove(passcode);
						frame1.dispose(); // close window
						HomePage home = new HomePage(); // go into account
					}
